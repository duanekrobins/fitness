...
This script also demonstrates how to automate data entry from a CSV file.
...
if len(sys.argv) != 3:
    print("Usage: python scriptname.py <output_file_path> <csv_file_path>")
    sys.exit(1)

# Get the output file path and CSV file path from the command-line arguments
...
# Load the workbook to append the data from CSV
workbook = load_workbook(output_file_path)
health_metrics_sheet = workbook['Health Metrics']

# Read data from the CSV file
csv_data = pd.read_csv(csv_file_path)

# Append the CSV data to the 'Health Metrics' sheet
for row in dataframe_to_rows(csv_data, index=False, header=True):
    health_metrics_sheet.append(row)

# Save the workbook with the appended data
workbook.save(output_file_path)

print(f"CSV data appended and saved in: {output_file_path}")
...
plt.ylabel('Activity')
plt.tight_layout()
plt.savefig('Activity_Data_Barplot.png')
...
plt.tight_layout()
plt.savefig('Sleep_Data_Barplot.png')
